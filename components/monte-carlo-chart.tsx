...
export default function MonteCarloChart({
  ...
  const barFillTarget = isDark ? "#347a55" : "#9aa5c0";
    ...
    <div>
      ...
      >
        ...
        {targetPlus !== null && (
          <>
            <line
              x1={xScale(targetPlus)}
              y1={pad.top}
              x2={xScale(targetPlus)}
              y2={pad.top + chartH}
              stroke={targetColor}
              strokeWidth={1.5}
              strokeDasharray="6 3"
        ...
        {targetMinus !== null && (
          <>
            <line
              x1={xScale(-targetMinus)}
              y1={pad.top}
              x2={xScale(-targetMinus)}
              y2={pad.top + chartH}
              stroke={targetColor}
              strokeWidth={1.5}
              strokeDasharray="6 3"
      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-3 mt-4">
        ...
        <Stat
          label="MC Yield"
          value={
            stats.yieldPct !== null ? `${stats.yieldPct.toFixed(4)}%` : "\u2014"
          }
        />
      </div>

      {/* Legend */}
      <div className="flex flex-wrap gap-4 mt-3 text-[10px] text-navy-500 dark:text-forest-400">
        ...
        <span className="flex items-center gap-1">
          <span
            className="inline-block w-3 h-0.5 rounded"
            style={{ background: barFillTarget }}
          />
          {`\u00b1${sigmaK}\u03c3`} bounds
        </span>
        {targetPlus !== null && (
          <span className="flex items-center gap-1">
            <span
              className="inline-block w-3 h-2 rounded-sm opacity-50"
              style={{ background: targetColor }}
            />
            Out of spec
          </span>
  ...
  return (
    <div className="text-center">
      ...
      <p className="text-sm font-mono font-semibold text-navy-800 dark:text-forest-100">
        {value}
        {unit && (
          <span className="text-[10px] font-normal text-navy-400 dark:text-forest-400 ml-0.5">
            {unit}
          </span>
        )}
      </p>
